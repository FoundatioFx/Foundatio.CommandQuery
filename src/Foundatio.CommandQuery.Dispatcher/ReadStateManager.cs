...
/// Extends <see cref="StateManager{TModel}"/> to include asynchronous data loading capabilities from a data service.
/// </summary>
...
/// <remarks>
/// The <see cref="ReadStateManager{TKey, TModel}"/> class is designed for scenarios where you need to load and manage
...
{
    /// <summary>
    /// Initializes a new instance of the <see cref="ReadStateManager{TKey, TModel}"/> class.
    /// </summary>
    ...
    public ReadStateManager(IDispatcherDataService dataService)
    {
    ...
    /// </para>
    /// <para>
    /// During the load operation, <see cref="IsBusy"/> is set to <c>true</c> and <see cref="StateManager{TModel}.NotifyStateChanged"/>
    /// is called to notify subscribers. Upon completion, <see cref="IsBusy"/> is reset to <c>false</c> and
    /// <see cref="StateManager{TModel}.NotifyStateChanged"/> is called again.
    /// </para>
    /// <para>
    /// If the load operation succeeds, the loaded model is automatically set using the inherited <see cref="StateManager{TModel}.Model"/> property.
